...
if stack exec ormolu -- --mode check src/*/*.hs test/Spec.hs test/Unit/*/*.hs 2>/dev/null; then
    print_success "Code formatting is correct"
else
    print_error "Code formatting issues found!"
    print_warning "Run: stack exec ormolu -- --mode inplace src/*/*.hs test/Spec.hs test/Unit/*/*.hs"
    exit 1
fi
